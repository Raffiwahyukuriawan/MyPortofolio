const SERVICES = [
  { title:'Web Development',    icon:'🌐', color:'#06B6D4', desc:'React, Next.js, Node.js — responsive & fast web apps.' },
  { title:'Mobile Development', icon:'📱', color:'#A855F7', desc:'Flutter & Kotlin — native Android dan cross-platform.' },
  { title:'Desktop Apps',       icon:'💻', color:'#10B981', desc:'Python, Electron — aplikasi desktop yang solid.' },
  { title:'UI/UX Design',       icon:'🎨', color:'#FBBF24', desc:'Figma, prototyping, design systems yang clean.' },
];
      ...
      <div style={{ padding:'6rem 0', borderTop:'1px solid rgba(168,85,247,.12)' }}>
        <div className="container">
          ...
          <div className="d-flex align-items-center gap-3 mb-5" data-aos="fade-right">
            <span className="section-label" style={{ color:'#06B6D4' }}>02 /</span>
            <span className="section-label" style={{ color:'rgba(240,240,255,.35)' }}>About Me</span>
          ...
          <div className="row g-5 align-items-start">
            ...
            <div className="col-lg-5" data-aos="fade-up" data-aos-delay="100">
              <h2 className="display-neo mb-4" style={{ fontSize:'clamp(2.5rem,6vw,4rem)' }}>
                Who Am<br /><span className="grad-text">I?</span>
              </h2>
            ...
            <div className="col-lg-7" data-aos="fade-up" data-aos-delay="200">
              <div className="row g-3">
                {SERVICES.map((s,i) => (
                  <div key={s.title} className="col-sm-6" data-aos="zoom-in" data-aos-delay={200+i*80}>
                    <div className="service-card" style={{ borderColor:`rgba(${hexToRgb(s.color)}, 0.2)`, color:s.color }}>
                      <span className="service-icon">{s.icon}</span>
                      <h5 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontWeight:700, textTransform:'uppercase', fontSize:'.95rem', color:'#F0F0FF', marginBottom:'.5rem' }}>
                        {s.title}
                      </h5>
                      <p style={{ color:'rgba(240,240,255,.45)', fontSize:'.8rem', margin:0, lineHeight:1.7 }}>{s.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="row g-3 mt-1">
                {[
                  { val:'🚌', text:'Bus miniatur collector', color:'#FBBF24' },
                  { val:'🏸', text:'Badminton player', color:'#10B981' },
                  { val:'📚', text:'SMK RPL student', color:'#06B6D4' },
                  { val:'🌏', text:'Based in Bandung', color:'#A855F7' },
                ].map((f) => (
                  <div key={f.text} className="col-6 col-sm-3">
                    <div style={{ textAlign:'center', padding:'1rem', background:'rgba(10,10,31,.5)', border:`1px solid rgba(${hexToRgb(f.color)},.15)`, borderRadius:3 }}>
                      <div style={{ fontSize:'1.5rem', marginBottom:'.4rem' }}>{f.val}</div>
                      <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:'.6rem', color:f.color, textTransform:'uppercase', letterSpacing:'.12em' }}>{f.text}</div>
                    </div>
                  </div>
                ))}
              </div>
